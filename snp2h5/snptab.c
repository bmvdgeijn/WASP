...
SNPTab *snp_tab_new(hid_t h5file, const char *chrom_name,
  ...
  tab->field_type[2] = H5T_NATIVE_LONG; /* pos */
  tab->field_type[3] = tab->allele_type; /* allele1 */
  tab->field_type[4] = tab->allele_type; /* allele2 */
  ...
  tab->field_size[2] = sizeof(snp_desc.pos);
  tab->field_size[3] = sizeof(snp_desc.allele1);
  tab->field_size[4] = sizeof(snp_desc.allele2);
    
  /* offsets of each field */
  tab->field_offset[0] = HOFFSET(SNP, name);
  tab->field_offset[1] = HOFFSET(SNP, chrom);
  tab->field_offset[2] = HOFFSET(SNP, pos);
  tab->field_offset[3] = HOFFSET(SNP, allele1);
  tab->field_offset[4] = HOFFSET(SNP, allele2);
